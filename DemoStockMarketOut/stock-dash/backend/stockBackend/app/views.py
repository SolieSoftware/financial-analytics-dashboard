...
app = FastAPI(lifespan=lifespan)  # Fixed: FastAPI not FASTAPI
...
async def get_ticker_historical_data(ticker: str):  # Fixed: Added ticker parameter
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch history for {ticker}: {str(e)}")
...
async def get_ticker_info_data(ticker: str):  # Fixed: Added ticker parameter
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch info for {ticker}: {str(e)}")
